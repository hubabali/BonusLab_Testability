...
public class QueryInvoicesDAO {
    ...
    public void clear() {
        connection.withSql(() -> {  // Executes SQL operations within the database connection
            try (var ps = connection.getConnection().prepareStatement("DELETE FROM invoice")) {  // SQL to delete all rows
                ps.execute();  // Executes the delete query
                connection.getConnection().commit();  // Commits the transaction
            }
